...
param p;
param cons;
param is_integer default 0;
param lb {i in 1..n}default 0; 
param ub {i in 1..n}default 0; 
...
param vp {s in 1..n} default 0;
param assoc_prod{s in 1..n_y, t in 1..2} default 0;
param objlb default -1.e300;

#for certifying the convexity of the new objectif function
param lambda default 1e-6;
param vp_cert default 0;
param prec default 1.e-6;
var x {i in 1..n};


minimize obj :
sum {i in 1..n,j in 1..n}  Q[i,j] * x[i] * x[j] + sum {i in 1..n-n_y }  2*lambda* x[i] * x[i]  
+ sum {i in 1..n-n_y}c[i] * x[i] + sum{i in n-n_y+1..n} c[i]*x[i] - sum{i in n-n_y+1..n : assoc_prod[i-n+n_y,1] =assoc_prod[i-n+n_y,2]} 2*lambda*x[i] +cons;
...
subject to cor2{s in n-n_y+1..n}:
...
#subject to cor5{s in n-n_y+1..n : assoc_prod[s-n+n_y,1] == assoc_prod[s-n+n_y,2] and is_integer==1}:
